...
#[derive(Debug, Clone)]
pub struct BalanceOfPowerFeatures {
    pub value: f64,
    pub trend: Option<Trend>,
    pub signal: StrategySignal,
}

impl Default for BalanceOfPowerFeatures {
    fn default() -> Self {
        return Self {
            value: f64::NAN,
            trend: None,
...
impl Features for BalanceOfPowerFeatures {
...
pub struct BalanceOfPowerFeatureBuilder {
    pub ctx: Context,
    pub inner: BalanceOfPower,
    pub inner_strategy: BalanceOfPowerStrategy,
    features: BalanceOfPowerFeatures,
}

impl BalanceOfPowerFeatureBuilder {
    pub fn new(
        ctx: Context,
        inner: BalanceOfPower,
        inner_strategy: BalanceOfPowerStrategy,
    ) -> Self {
        return Self {
            inner,
            inner_strategy,
            ctx,
            features: BalanceOfPowerFeatures::default(),
        };
    }
}

impl IncrementalFeatureBuilder<BalanceOfPowerFeatures> for BalanceOfPowerFeatureBuilder {
    const NAMESPACE: &'static str = "ta::third_party::tradingview:::balance_of_power";
}

impl Incremental<(), BalanceOfPowerFeatures> for BalanceOfPowerFeatureBuilder {
    fn next(&mut self, _: ()) -> BalanceOfPowerFeatures {
...
impl Incremental<(), Box<dyn Features>> for BalanceOfPowerFeatureBuilder {
    fn next(&mut self, _: ()) -> Box<dyn Features> {
        return Box::new(Incremental::<(), BalanceOfPowerFeatures>::next(self, ()));
    }
}
