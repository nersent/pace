...
#[derive(Debug, Clone)]
pub struct ConnorsRelativeStrengthIndexFeatures {
    pub value: f64,
    pub trend: Option<Trend>,
    pub signal: StrategySignal,
}

impl Default for ConnorsRelativeStrengthIndexFeatures {
    fn default() -> Self {
        return Self {
            value: f64::NAN,
            trend: None,
...
impl Features for ConnorsRelativeStrengthIndexFeatures {
...
pub struct ConnorsRelativeStrengthIndexFeatureBuilder {
    pub ctx: Context,
    pub inner: ConnorsRelativeStrengthIndex,
    pub inner_strategy: ConnorsRelativeStrengthIndexStrategy,
    features: ConnorsRelativeStrengthIndexFeatures,
}

impl ConnorsRelativeStrengthIndexFeatureBuilder {
    pub fn new(
        ctx: Context,
        inner: ConnorsRelativeStrengthIndex,
        inner_strategy: ConnorsRelativeStrengthIndexStrategy,
    ) -> Self {
        return Self {
            inner,
            inner_strategy,
            ctx,
            features: ConnorsRelativeStrengthIndexFeatures::default(),
        };
    }
}

impl IncrementalFeatureBuilder<ConnorsRelativeStrengthIndexFeatures>
    for ConnorsRelativeStrengthIndexFeatureBuilder
{
    const NAMESPACE: &'static str =
        "ta::third_party::tradingview:::connors_relative_strength_index";
}

impl Incremental<(), ConnorsRelativeStrengthIndexFeatures>
    for ConnorsRelativeStrengthIndexFeatureBuilder
{
    fn next(&mut self, _: ()) -> ConnorsRelativeStrengthIndexFeatures {
...
impl Incremental<(), Box<dyn Features>> for ConnorsRelativeStrengthIndexFeatureBuilder {
    fn next(&mut self, _: ()) -> Box<dyn Features> {
        return Box::new(Incremental::<(), ConnorsRelativeStrengthIndexFeatures>::next(self, ()));
    }
}
