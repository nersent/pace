...
mod tests {
    ...
    use crate::{
        base::{
            ...
            strategy::{
                metrics::equity_metric::{EquityMetric, EquityMetricConfig},
                ...
                trade::{
                    compute_fill_size, compute_pnl, compute_return, compute_trade_pnl,
                    trade_direction_from_f64, Trade, TradeDirection,
                },
            },
        },
    ...
    // fn strategy_runner() {
    //     let (_df, ctx) =
    //         Fixture::raw("base/strategy/metrics/tests/fixtures/continous_next_bar.csv");
    //     // let expected = _df.column("_target_openprofit_").unwrap().to_f64();
    //     let trades = _df.column("trade").unwrap().to_f64();
    ...
    //     let mut snapshot =
    //         ComponentTestSnapshot::<((Option<f64>, Option<f64>, Option<f64>))>::new();
    ...
    //         let ctx = ctx.get();
    //         let tick = ctx.current_tick;
    //         return trade_direction_from_f64(trades[tick]);
    //     });

    //     let actual: Vec<Option<(Option<f64>, Option<f64>, Option<f64>)>> = result
    //         .metrics_history
    //         .iter()
    //         .map(|x| {
    //             Some((
    //                 Some(x.equity.unwrap().trade_pnl),
    //                 Some(x.equity.unwrap().net_profit),
    //                 Some(x.equity.unwrap().capital),
    //             ))
    //         })
    //         .collect::<Vec<Option<(Option<f64>, Option<f64>, Option<f64>)>>>();

    //     // let actual = result
    //     //     .metrics_history
    //     //     .iter()
    //     //     .map(|x| Some(x.equity.unwrap().trade_pnl))
